...
<div
	class="h-[100vh] transition-colors duration-300 {currentTheme === 'dark'
		? 'bg-black'
		: 'bg-white'}"
...
>
	...
	<div class="w-full py-2">
		<ul
			...
			class="relative mx-auto flex w-fit rounded-full border-1 {currentTheme === 'dark'
				? 'border-white text-white'
				: 'border-black text-black'} p-1"
		>
			{#each navs as item}
				<button
					use:positionMotion
					class="relative z-10 block cursor-pointer px-3 py-1.5 text-xs text-white uppercase mix-blend-difference md:px-5 md:py-3 md:text-base"
			...
			>
				<li
					use:motion
					class="absolute z-0 h-7 rounded-full {currentTheme === 'light'
						? 'bg-black'
						: 'bg-white'} md:h-12"
				></li>
			</Motion>
	...
	{#if openModal === 'products'}
		<ProductsManagement {currentTheme} on:close={() => (openModal = null)} />
	{:else if openModal === 'orders'}
		<OrdersManagement {currentTheme} on:close={() => (openModal = null)} />
	{:else if openModal === 'users'}
		<UsersManagement {currentTheme} on:close={() => (openModal = null)} />
	{/if}
</div>
